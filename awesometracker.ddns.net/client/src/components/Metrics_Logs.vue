				...
				<table class="table table-hover">
					...
					<thead>
						...
						<tr>
							...
							<th @click='order($event, "trackerLogs.code")'>Code<i class="fas fa-sort float-right" id="searchIcon"></i></th>
		...
		methods: {
			...
			order: async function(e, orderBy) {
				...
				if (e.target.childNodes.length == 1) {
					...
					let result = await Axios.get(`https://awesometracker.ddns.net/dashboard/data/2?orderBy=${orderBy}`)
			...
			deleteLog: async function (log) {
				...
				let result = await Axios.post(`https://awesometracker.ddns.net/dashboard/deleteLog`, {logCode: log['trackerLogs.code']});

				if (result.data.status == 'ok') {

					let msg = result.data.msg;

					result = await Axios.get(`https://awesometracker.ddns.net/dashboard/data/2?orderBy=${this.orderBy}`)
